...
// Interface para tipagem do vídeo
interface TipsVideo {
  id: number;
  videoUrl: string;
  category: string;
...
// Componente LazyVideo otimizado
const LazyVideo = memo(({ video, index }: { video: TipsVideo; index: number }) => {
  const { videoRef, isIntersecting } = useLazyVideo({ rootMargin: '100px' });

  return (
    <video
      ref={videoRef}
      src={isIntersecting ? video.videoUrl : undefined}
              ...
              {tipsVideos.map((video, index) => (
                ...
                >
                  <div className="relative h-[36rem] sm:h-[32rem] rounded-2xl overflow-hidden shadow-xl transition-all duration-300 hover:shadow-2xl aspect-[9/16] w-full max-w-[320px] sm:max-w-[280px] mx-auto">
                    ...
                    {/* Video Preview */}
                    <LazyVideo video={video} index={index} />
